...
export default function Header() {
  ...
  return (
    <>
      <div className="top-note">ارسال سفارش از بازار بزرگ تهران <span>•</span> تضمین در کیفیت</div>
      <header className="site-header">
        <div className="container header-inner">
          ...
          <div className="header-actions">
            <button className="search-pill" onClick={() => setSearchOpen(true)}><Search size={19} /><span>جست‌وجوی لباس...</span></button>
            ...
            <Link className="icon-button cart-target" href="/cart" aria-label={`سبد خرید، ${cartCount} محصول`}><ShoppingBag />{cartCount > 0 && <b>{cartCount}</b>}</Link>
          </div>
        </div>
      </header>
          ...
          <motion.div className="search-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && setSearchOpen(false)}>
            <motion.div className="search-panel" initial={{ opacity: 0, y: -20, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -12 }}>
              <div className="search-input-wrap">
                <Search />
                <input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="نام، رنگ یا دسته‌بندی محصول" />
                {query && <button onClick={() => setQuery('')} aria-label="پاک کردن"><X size={18} /></button>}
              </div>
              <div className="search-meta"><span>{query ? `${results.length.toLocaleString('fa-IR')} نتیجه` : 'پیشنهادهای محبوب'}</span><button onClick={() => setSearchOpen(false)}>بستن</button></div>
              <div className="search-results">
                {results.length ? results.map((product) => (
                  <Link key={product.id} href={`/products/${product.slug}`} onClick={() => setSearchOpen(false)}>
                    ...
                    <span><strong>{product.name}</strong><small>{formatPrice(product.price)} تومان</small></span>
                  </Link>
                )) : <div className="empty-search">محصولی با این عبارت پیدا نشد.</div>}
              </div>
              <Link className="search-all" href={`/products${query ? `?q=${encodeURIComponent(query)}` : ''}`} onClick={() => setSearchOpen(false)}>مشاهده همه محصولات</Link>
            </motion.div>
          </motion.div>
        )}
        {menuOpen && (
          <motion.div className="drawer-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMenuOpen(false)}>
            <motion.aside className="mobile-drawer" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 280 }} onClick={(event) => event.stopPropagation()}>
              ...
              <div className="drawer-contact"><small>هماهنگی سفارش</small><strong>۰۹۱۲۳۰۰۵۰۶۵</strong><span>مسئول سفارش: امینی</span></div>
            </motion.aside>
          </motion.div>
        )}
      </AnimatePresence>
