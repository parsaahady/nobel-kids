...
export default function ContactClient() {
  ...
  return (
    <>
      <section className="contact-hero"><div className="container"><div><span>ارتباط با نوبل</span><h1>برای سفارش،<br /><em>کنار شماییم.</em></h1><p>اطلاعات فروشگاه و راه‌های مستقیم هماهنگی سفارش را اینجا ببینید.</p></div><div className="contact-hero-mark"><span>🐢</span><strong>NOBEL</strong><small>kids</small></div></div></section>
      <section className="container contact-cards">
        ...
        <a href="tel:09123005065"><IconBubble tone="mint" size="lg"><Smartphone /></IconBubble><span><small>موبایل سفارش</small><strong dir="ltr">{siteInfo.mobile}</strong><em>مسئول هماهنگی: {siteInfo.manager}</em></span></a>
        <a href={siteInfo.instagramUrl} target="_blank" rel="noreferrer"><IconBubble tone="butter" size="lg"><Instagram /></IconBubble><span><small>اینستاگرام</small><strong dir="ltr">{siteInfo.instagram}</strong><em>مشاهده تازه‌ترین مدل‌ها</em></span></a>
      </section>
      <section className="container contact-main">
        ...
        <div className="contact-form-card"><div className="form-card-head"><span><MessageCircle /></span><div><small>پیام مستقیم</small><h2>فرم تماس</h2></div></div><p>موضوع و اطلاعات تماس را بنویسید؛ پیام شما برای مسئول هماهنگی ارسال می‌شود.</p><form onSubmit={submit}><div><label><span>نام و نام خانوادگی</span><input name="name" required placeholder="نام شما" /></label><label><span>شماره تماس</span><input name="phone" required inputMode="tel" dir="ltr" placeholder="09xxxxxxxxx" /></label></div><label><span>موضوع</span><select name="subject"><option>هماهنگی سفارش عمده</option><option>پیگیری سفارش</option><option>پرسش درباره محصول</option><option>همکاری با نوبل</option></select></label><label><span>متن پیام</span><textarea name="message" required placeholder="پیام خود را کوتاه و دقیق بنویسید" /></label><button className="primary-button" disabled={loading}>{loading ? <><span className="custom-loader" /> در حال ارسال</> : <>ارسال پیام <Send /></>}</button></form><AnimatePresence>{sent && <motion.div className="form-success" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><Check /> پیام شما ثبت شد؛ برای پیگیری سریع‌تر می‌توانید با شماره موبایل تماس بگیرید.</motion.div>}</AnimatePresence></div>
      </section>
      <section className="container contact-about"><div><span className="tiny-label">درباره ما</span><h2>نوبل کیدز؛ از ۱۳۹۶</h2><p>{siteInfo.history}</p></div><div><span><strong>تضمین</strong><small>در کیفیت</small></span><i>🐢</i><p>تولید و پخش عمده<br />پوشاک بچه‌گانه</p></div></section>
    </>
  );
}
