...
import { AlertCircle, Check, Instagram, MapPin, MessageCircle, Phone, Send, Smartphone } from 'lucide-react';
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post('/api/contact', {
        ...
        message: `فروشگاه: ${String(form.get('business'))} — ${String(form.get('message'))}`,
      });
      setSent(true); formElement.reset();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'ارسال پیام ممکن نشد');
  ...
  return (
    <>
      ...
      <section className="container contact-main">
        ...
        <div className="contact-form-card"><div className="form-card-head"><span><MessageCircle /></span><div><small>پیام مستقیم</small><h2>فرم تماس</h2></div></div><p>نام فروشگاه و موضوع سفارش عمده را بنویسید؛ پیام مستقیم برای مسئول هماهنگی ارسال می‌شود.</p><form onSubmit={submit}><div><label><span>نام و نام خانوادگی</span><input name="name" required placeholder="نام شما" /></label><label><span>شماره تماس</span><input name="mobile" required inputMode="tel" dir="ltr" placeholder="09xxxxxxxxx" pattern="09[0-9]{9}" /></label></div><label><span>نام فروشگاه یا مجموعه</span><input name="business" required placeholder="نام مجموعه شما" /></label><label><span>موضوع</span><select name="subject"><option>دریافت موجودی و سفارش عمده</option><option>پیگیری پیش‌فاکتور عمده</option><option>پرسش درباره پک و سایزبندی</option><option>درخواست همکاری فروش</option></select></label><label><span>متن پیام</span><textarea name="message" required placeholder="مدل، تعداد پک یا پرسش خود را کوتاه و دقیق بنویسید" /></label><button className="primary-button" disabled={loading}>{loading ? <><span className="custom-loader" /> در حال ارسال</> : <>ارسال پیام <Send /></>}</button></form><AnimatePresence>{sent && <motion.div className="form-success" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><Check /> پیام شما ثبت شد؛ برای پیگیری سریع‌تر می‌توانید با شماره موبایل تماس بگیرید.</motion.div>}{error && <motion.div className="form-error" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><AlertCircle /> {error}</motion.div>}</AnimatePresence></div>
      </section>
