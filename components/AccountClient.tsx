...
const tabs = [
  ...
  { id: 'orders', label: 'پیش‌فاکتورها', icon: Package },
...
export default function AccountClient() {
    ...
    <section className="account-auth-section">
      <div className="container account-auth-grid">
        <motion.div className="auth-card" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <small>{register ? 'عضویت همکاران نوبل' : 'خوش آمدید'}</small>
          <h1>{register ? 'ساخت حساب همکاری' : 'ورود همکاران نوبل'}</h1>
          <p>{register ? 'اطلاعات مسئول خرید را برای پیگیری پیش‌فاکتورها وارد کنید.' : 'برای مدیریت پیش‌فاکتورها، آدرس ارسال و مدل‌های منتخب وارد شوید.'}</p>
          <form onSubmit={login}>
            {register && <><label><span>نام و نام خانوادگی مسئول خرید</span><input name="name" required placeholder="نام شما" /></label><label><span>نام فروشگاه یا مجموعه</span><input name="business" required placeholder="نام فروشگاه" /></label></>}
        ...
        <div className="auth-visual"><span className="auth-tag">NOBEL <small>kids</small></span><div className="auth-blob"><span>🐢</span><strong>شبکه<br />همکاران نوبل</strong></div><p>مدل‌های منتخب، پیش‌فاکتورها و آدرس‌های ارسال همیشه در دسترس است.</p></div>
      </div>
    </section>
  );
  ...
  return (
    <>
      <section className="soft-page-head account-head"><div className="container"><span>حساب کاربری</span><h1>سلام، {name}</h1><p>پیش‌فاکتورها و انتخاب‌های عمده خود را از اینجا مدیریت کنید.</p></div></section>
      <div className="container account-layout">
        <aside className="account-sidebar"><div className="account-person"><span>{name.charAt(0)}</span><div><strong>{name}</strong><small>عضو شبکه همکاران نوبل</small></div></div><nav>{tabs.map(({ id, label, icon: Icon }) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}><Icon />{label}{id === 'wishlist' && wishlist.length > 0 && <b>{wishlist.length.toLocaleString('fa-IR')}</b>}<ChevronLeft /></button>)}</nav><button className="logout-button" onClick={() => { localStorage.removeItem('nobel-session'); setLoggedIn(false); }}><LogOut /> خروج از حساب</button></aside>
        <section className="account-content">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'orders' && <motion.div key="orders" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}><div className="account-content-head"><div><small>پیگیری خرید</small><h2>پیش‌فاکتورهای من</h2></div></div><div className="account-empty"><span><Package /></span><h3>هنوز پیش‌فاکتوری ثبت نکرده‌اید</h3><p>پس از ثبت درخواست عمده، وضعیت تأیید و آماده‌سازی اینجا نمایش داده می‌شود.</p><Link className="secondary-button" href="/products">انتخاب مدل‌های عمده</Link></div></motion.div>}
