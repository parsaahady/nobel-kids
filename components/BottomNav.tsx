  ...
  return (
    <nav className="bottom-nav" aria-label="منوی موبایل">
      {items.map((item) => {
        ...
        return <Link key={item.href} href={item.href} className={`${active ? 'active' : ''} ${item.href === '/cart' ? 'cart-target' : ''}`}><span><Icon />{Boolean(item.count) && <b>{Number(item.count).toLocaleString('fa-IR')}</b>}</span><small>{item.label}</small></Link>;
      })}
    </nav>
  );
}
