...
const orders = [
  ...
  { id: 'NBL-10427', customer: 'علی رضایی', date: '۲۶ مرداد ۱۴۰۵', count: 2, total: 1826000, status: 'ارسال‌شده' },
...
export default function AdminPanel() {
  ...
  const saveProduct = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (modal === 'edit' && editing) {
      ...
      setList((prev) => [{ ...base, id: Math.max(...prev.map((p) => p.id)) + 1, slug: `new-${Date.now()}`, name: String(data.get('name')), shortName: String(data.get('name')), price: Number(data.get('price')), stock: Number(data.get('stock')), image: '/products/photo_20.webp' }, ...prev]);
      notify('محصول جدید اضافه شد');
    }
  ...
  return (
    <section className="admin-shell container-wide">
      ...
      <main className="admin-main">
        <header className="admin-head"><div><small>پنل مدیریت نوبل کیدز</small><h1>{menu.find((item) => item.id === tab)?.label}</h1></div><button className="primary-button" onClick={() => { setEditing(null); setModal('add'); }}><Plus /> محصول جدید</button></header>

        {tab === 'dashboard' && <div className="admin-dashboard"><div className="admin-stats"><div><span className="admin-stat-icon mint"><CircleDollarSign /></span><p><small>فروش این ماه</small><strong>۴۸٬۹۲۰٬۰۰۰ <em>تومان</em></strong><span className="positive"><TrendingUp /> ۱۲٪ رشد</span></p></div><div><span className="admin-stat-icon rose"><ShoppingBag /></span><p><small>سفارش‌های جدید</small><strong>۲۸ <em>سفارش</em></strong><span>۷ سفارش امروز</span></p></div><div><span className="admin-stat-icon butter"><Box /></span><p><small>محصولات فعال</small><strong>{list.length.toLocaleString('fa-IR')} <em>مدل</em></strong><span>{lowStock.length.toLocaleString('fa-IR')} کم‌موجود</span></p></div><div><span className="admin-stat-icon sky"><PackageCheck /></span><p><small>ارسال‌شده</small><strong>۲۱ <em>بسته</em></strong><span>این ماه</span></p></div></div>
          <div className="admin-two-column"><section className="admin-card"><div className="admin-card-head"><div><small>وضعیت اخیر</small><h2>آخرین سفارش‌ها</h2></div><button onClick={() => setTab('orders')}>مشاهده همه</button></div><OrderTable /></section><section className="admin-card low-stock-card"><div className="admin-card-head"><div><small>نیازمند توجه</small><h2>موجودی کم</h2></div><span>{lowStock.length.toLocaleString('fa-IR')} مورد</span></div>{lowStock.slice(0, 4).map((product) => <div className="low-stock-row" key={product.id}><span><Image src={product.image} alt={product.name} fill sizes="52px" /></span><p><strong>{product.shortName}</strong><small>NBL-{String(product.id).padStart(3, '0')}</small></p><b>{product.stock.toLocaleString('fa-IR')} عدد</b></div>)}</section></div>
        </div>}

        {tab === 'products' && <section className="admin-card products-admin-card"><div className="admin-list-tools"><div className="admin-search"><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="جست‌وجوی محصول" /></div><select><option>همه دسته‌ها</option><option>دخترانه</option><option>پسرانه</option></select><div className="admin-count">{filtered.length.toLocaleString('fa-IR')} محصول</div></div><div className="admin-product-table"><div className="admin-tr admin-th"><span>محصول</span><span>دسته‌بندی</span><span>قیمت</span><span>موجودی</span><span>وضعیت</span><span /></div>{filtered.map((product) => <div className="admin-tr" key={product.id}><div className="admin-product-cell"><span><Image src={product.image} alt={product.name} fill sizes="56px" /></span><p><strong>{product.name}</strong><small>NBL-{String(product.id).padStart(3, '0')}</small></p></div><span>{product.category}</span><span>{formatPrice(product.price)} تومان</span><span className={product.stock < 10 ? 'low' : ''}>{product.stock.toLocaleString('fa-IR')} عدد</span><span className="status active"><i /> فعال</span><div className="row-actions"><button onClick={() => openEdit(product)}><Edit3 /></button><button onClick={() => remove(product.id)}><Trash2 /></button></div></div>)}</div></section>}

        {tab === 'orders' && <section className="admin-card orders-admin-card"><div className="admin-list-tools"><div><h2>سفارش‌های فروشگاه</h2><small>پیگیری و تغییر وضعیت سفارش‌ها</small></div><select><option>همه وضعیت‌ها</option><option>در حال آماده‌سازی</option><option>ارسال‌شده</option><option>تکمیل‌شده</option></select></div><OrderTable full /></section>}
